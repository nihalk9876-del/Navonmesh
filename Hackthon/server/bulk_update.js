...
// The long list of data (same as before)
const bulkDataStr = `
...
Jay gajanan|Jay Shrikant Potdukhe|student innovat
`;
...
const mapPS = (str) => {
    ...
    return 'Student Innovation'; // Default if unclear
};
...
async function run() {
    try {
        ...
        // Get all Srijan registrations
        const allSrijan = await Registration.find({ event: 'Srijan (Hackathon)' });
        console.log(`Total Srijan teams in DB: ${allSrijan.length}`);
        ...
        for (const update of updates) {
            // Find by team name (case-insensitive fuzzy)
            let reg = allSrijan.find(r => 
                r.teamName.toLowerCase().trim() === update.teamName.toLowerCase().trim()
            );

            // If not found, try by leader name
            if (!reg) {
                reg = allSrijan.find(r => 
                    r.leaderName.toLowerCase().trim() === update.leaderName.toLowerCase().trim()
                );
            }

            if (reg) {
                if (!foundIds.has(reg._id.toString())) {
                    await Registration.findByIdAndUpdate(reg._id, {
                        problemStatement: update.ps,
                        psEdited: true
                    });
                    foundIds.add(reg._id.toString());
                    foundCount++;
                }
            } else {
                missedList.push(`${update.teamName} (${update.leaderName})`);
                missedCount++;
            }
        }

        console.log(`Updated ${foundCount} unique teams.`);
        console.log(`Could not find ${missedCount} teams from the list.`);
        // Identify teams that DID NOT fill the form
        const didNotFill = allSrijan.filter(r => !foundIds.has(r._id.toString()));
        console.log(`Total teams that DID NOT fill the form: ${didNotFill.length}`);
        
        console.log('\n--- LIST OF TEAMS THAT DID NOT FILL THE FORM (COPIABLE) ---');
        didNotFill.forEach((r, i) => {
            console.log(`[${i+1}] ${r.teamName} - ${r.leaderName} (${r.leaderEmail})`);
        });
